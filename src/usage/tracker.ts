...
// 用量追踪器，用于记录每一次得API调用成本和用量明细，持久化到日志中
 ...
 * 统一不同厂商API返回的usage格式，转换成自定义的setupUsage格式
 * 便于UsageTracker统一处理所有厂商的返回值
 *
